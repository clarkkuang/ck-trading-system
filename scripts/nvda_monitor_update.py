#!/usr/bin/env python3
"""Weekly NVDA framework monitor update.

Collects DAILY closes for the watch tickers (NVDA/AMD/AVGO/TSM/SMH),
sanity-checks the quarterly fundamentals, evaluates the three-layer
buy/sell rules, and persists to git-tracked data/monitoring/nvda/.

Run weekly (GitHub Action: .github/workflows/nvda-monitor.yml) or manually:

    .venv/bin/python scripts/nvda_monitor_update.py
    .venv/bin/python scripts/nvda_monitor_update.py --backfill   # one-off seeding
...
--backfill (local-only, one-off):
    * seeds full daily price history: from the local gitignored
      ...
      yfinance fetch (TSM/SMH have no local file)
    * seeds fundamentals.json / scenarios.json from nvda_config defaults
...
from ck_trading.monitoring.nvda_pipeline import nvda_store, run_weekly_update
...
def backfill(store) -> None:
    ...
    if store.load_json("fundamentals") is None:
        ...
        log("  seeded fundamentals.json (2026Q1 est + 2026Q2 actuals)")
    if store.load_json("scenarios") is None:
        ...
        log("  seeded scenarios.json (weighted fair $223.75)")
...
def main() -> int:
    ...
    log("=== NVDA Framework Monitor Update ===")
    ...
    meta = MetadataStore()
    job_id = meta.log_job_start("nvda_monitor_update")
