#!/usr/bin/env python3
"""Weekly INTC framework monitor update.

Collects DAILY closes for the watch tickers (INTC/AMD/SMH/SPY),
sanity-checks the quarterly fundamentals, evaluates the three-layer
buy/sell rules (foundry-turnaround thesis), and persists to git-tracked data/monitoring/intc/.

Run weekly (GitHub Action: .github/workflows/intc-monitor.yml) or manually:

    .venv/bin/python scripts/intc_monitor_update.py
    .venv/bin/python scripts/intc_monitor_update.py --backfill   # one-off seeding
...
--backfill (local-only, one-off):
    * seeds full daily price history: from the local gitignored
      ...
      yfinance fetch (AMD/SMH/SPY/INTC — local files vary; yfinance fallback)
    * seeds fundamentals.json / scenarios.json from intc_config defaults
...
from ck_trading.monitoring.intc_pipeline import intc_store, run_weekly_update
...
def backfill(store) -> None:
    ...
    from ck_trading.monitoring.intc_metrics import normalize_daily
    ...
    if store.load_json("scenarios") is None:
        ...
        log("  seeded scenarios.json (weighted fair $83.20)")
...
def main() -> int:
    ...
    log("=== INTC Framework Monitor Update ===")
    ...
    meta = MetadataStore()
    job_id = meta.log_job_start("intc_monitor_update")
