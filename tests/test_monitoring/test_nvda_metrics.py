...
class TestScenarios:
    def test_weighted_fair(self):
        # 2026-09-03: rebuilt, not re-weighted. Q2 FY27 put FY27E non-GAAP EPS
        # at ~$9.22 — the OLD table's most bullish EPS assumption ($9.0) was
        # already this year's actual, so the scenarios were re-anchored on
        # FY28E EPS. Prices moved because the earnings base did.
        assert weighted_fair_value(list(DEFAULT_SCENARIOS)) == pytest.approx(330.65)
