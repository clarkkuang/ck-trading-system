...
@st.cache_data(ttl=_TTL, show_spinner=False)
def load_nvda_monitoring(name: str) -> pl.DataFrame:
    from ck_trading.monitoring.nvda_pipeline import nvda_store

    return nvda_store().load(name)


@st.cache_data(ttl=_TTL, show_spinner=False)
def load_nvda_alerts() -> dict:
    from ck_trading.monitoring.nvda_pipeline import nvda_store

    return nvda_store().load_alerts()


@st.cache_data(ttl=_TTL, show_spinner=False)
def load_nvda_json(name: str) -> dict:
    from ck_trading.monitoring.nvda_pipeline import nvda_store

    return nvda_store().load_json(name, default={}) or {}


@st.cache_data(ttl=_TTL, show_spinner=False)
def load_nflx_monitoring(name: str) -> pl.DataFrame:
    from ck_trading.monitoring.nflx_pipeline import nflx_store

    return nflx_store().load(name)


@st.cache_data(ttl=_TTL, show_spinner=False)
def load_nflx_alerts() -> dict:
    from ck_trading.monitoring.nflx_pipeline import nflx_store

    return nflx_store().load_alerts()


@st.cache_data(ttl=_TTL, show_spinner=False)
def load_nflx_json(name: str) -> dict:
    from ck_trading.monitoring.nflx_pipeline import nflx_store

    return nflx_store().load_json(name, default={}) or {}
...
@st.cache_data(ttl=_TTL, show_spinner=False)
def load_tcnt_monitoring(name: str) -> pl.DataFrame:
    from ck_trading.monitoring.tcnt_pipeline import tcnt_store

    return tcnt_store().load(name)


@st.cache_data(ttl=_TTL, show_spinner=False)
def load_tcnt_alerts() -> dict:
    from ck_trading.monitoring.tcnt_pipeline import tcnt_store

    return tcnt_store().load_alerts()


@st.cache_data(ttl=_TTL, show_spinner=False)
def load_tcnt_json(name: str) -> dict:
    from ck_trading.monitoring.tcnt_pipeline import tcnt_store

    return tcnt_store().load_json(name, default={}) or {}
