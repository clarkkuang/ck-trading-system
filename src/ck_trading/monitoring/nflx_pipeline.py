"""Weekly orchestration for the NFLX framework monitor (4th instance).

Sections (each isolated, mirroring the nvda pipeline):

    prices        DAILY closes for NFLX/DIS/SPY/QQQ via yfinance
                  (daily — the trend-repair rule needs the 200-day SMA)
    fundamentals  read-only sanity pass over fundamentals.json
                  (dashboard/backfill are the only writers)
    alerts        evaluate NFLX_RULES -> alerts.json episodes

Nothing here may depend on the gitignored data/prices/ tree (absent in CI
checkouts) — historical seeding lives in scripts/nflx_monitor_update.py
--backfill only.
"""
...
def nflx_store() -> MonitoringStore:
    return MonitoringStore(
        settings.monitoring_dir / "nflx",
        dedupe_keys=NFLX_DEDUPE_KEYS,
        checklist_seed=DEFAULT_CHECKLIST,
    )


def build_nflx_series_provider(store: MonitoringStore):
    """SeriesProvider over the NFLX monitor's stored data."""
    ...
    def _indicators(ticker: str) -> pl.DataFrame:
        key = f"ind:{ticker}"
        if key not in cache:
            cache[key] = nflx_metrics.daily_indicators(_prices(), ticker)
        return cache[key]  # type: ignore[return-value]

    def provider(metric_key: str, dims: dict[str, str]) -> pl.DataFrame:
        ticker = dims.get("ticker", "")
        if metric_key == "nflx.price_weekly_close":
            return nflx_metrics.weekly_close_series(_prices(), ticker)
        if metric_key == "nflx.pct_vs_200dma":
            return nflx_metrics.weekly_sample(
                _indicators(ticker), "pct_vs_200dma"
            )
        if metric_key == "nflx.fundamental":
...
def run_weekly_update(
    *,
    ...
    store = store or nflx_store()
    result = PipelineResult(
        as_of=as_of,
        title="NFLX framework monitor",
    ...
    if skip_prices:
        ...
        try:
            ...
            clean = nflx_metrics.normalize_daily(daily)
            if clean.is_empty():
    ...
    try:
        ...
        if not quarters:
            ...
            errors = nflx_metrics.validate_fundamentals(quarters)
            if errors:
                result.outcomes.append(ComponentOutcome(
    ...
    try:
        ...
        rule_results = evaluate_rules(NFLX_RULES, provider)
