"""Pure metric functions for the NVDA framework monitor. No I/O.

Key difference from the att monitor: prices are stored DAILY because the
technical layer needs 60-trading-day rolling highs and the 200-day SMA
(matching the portfolio_dip.py calibration, which uses row windows).
Rule evaluation still happens on WEEKLY sampled series so the shared
engine's max_gap_days=14 semantics carry over unchanged.
"""
