...
7/27 事件:OpenAI $250B 担保谈判 + SK $500B 协议曝光,NVDA -5%、其 5Y CDS 单日
+14bp 至 82bp(该合约活跃交易以来最大单日涨幅),Oracle CDS 创周期新高。市场
重定价的不是需求,是"卖铲人给买铲人兜底"的循环融资结构(朗讯 2000 先例:
vendor financing 峰值约占营收 20%+)。

框架据此新增第四维度(不改既有三层与情景概率):
- `nvda_sell_financing_exposure_25`(trigger):客户融资敞口(担保+客户股权+承诺)
  ÷TTM 营收 ≥25% — OpenAI $250B 若签署对 ~$325B 年化营收 ≈77%,直接击穿
- `nvda_sell_ar_days_rising_2q`(advisory):应收账款天数连升 2 季 — 循环融资的
  会计指纹(收入质量)
- 清单月检:NVDA 自身 5Y CDS 水平+周变速(预警:单周 +10bp 或破 100bp)、
  Oracle/Meta CDS、AI 债 IG 利差 — NVDA CDS 是敞口比的实时代理,比 10-Q 快一个月

数据边界:两个字段自 8/26 的 10-Q(承诺与或有事项附注 + 应收明细)起回填,此前
insufficient_data;CDS 无免费数据源,只能人工月检/事件检。
